...
export function renderElevationTokens(tokens: Tokens, container: HTMLElement) {
  const categories = ["elevation"];
...
function createThemeCell(
  ...
  if (token) {
    ...
    if (token.$type === "boxShadow") {
      previewElement = `
        <div class="shadow-preview ${theme}">
            <div class="swatch" style="box-shadow: var(${cssVarName});"></div>
        </div>
      `;
    } else {
      previewElement = `
